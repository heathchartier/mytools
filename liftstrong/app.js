...
// ===== REST TIMER =====
var _rtMinSel = 1, _rtSecSel = 0;

function _buildWheel(innerId, values, selected, onPick) {
  var inner = document.getElementById(innerId);
  if (!inner) return;
  inner.innerHTML = '';
  var pad = document.createElement('div');
  pad.className = 'rt-wheel-item';
  inner.appendChild(pad.cloneNode(true));
  values.forEach(function(v) {
    var el = document.createElement('div');
    el.className = 'rt-wheel-item' + (v === selected ? ' selected' : '');
    el.textContent = String(v).padStart(2, '0');
    el.dataset.val = v;
    el.addEventListener('click', function() { onPick(v); });
    inner.appendChild(el);
  });
  inner.appendChild(pad.cloneNode(true));
  var wheel = inner.parentElement;
  var idx = values.indexOf(selected);
  if (idx >= 0) wheel.scrollTop = idx * 40;
}

function _initWheels() {
  var mins = [0,1,2,3,4,5];
  var secs = [0,5,10,15,20,25,30,35,40,45,50,55];
  _buildWheel('rt-wheel-min-inner', mins, _rtMinSel, function(v) {
    _rtMinSel = v;
    if (_rtMinSel === 5) _rtSecSel = 0;
    var secList = _rtMinSel === 5 ? [0] : secs;
    _buildWheel('rt-wheel-sec-inner', secList, _rtSecSel, function(sv) {
      _rtSecSel = sv; _applyWheel();
    });
    _attachScroll('rt-wheel-sec', _rtMinSel === 5 ? [0] : secs, function(sv) { _rtSecSel = sv; _applyWheel(); });
    _applyWheel();
  });
  _buildWheel('rt-wheel-sec-inner', secs, _rtSecSel, function(v) {
    _rtSecSel = v; _applyWheel();
  });
  _attachScroll('rt-wheel-min', mins, function(v) { _rtMinSel = v; _buildWheel('rt-wheel-min-inner', mins, _rtMinSel, function(mv){ _rtMinSel = mv; _applyWheel(); }); _applyWheel(); });
  _attachScroll('rt-wheel-sec', secs, function(v) { _rtSecSel = v; _buildWheel('rt-wheel-sec-inner', secs, _rtSecSel, function(sv){ _rtSecSel = sv; _applyWheel(); }); _applyWheel(); });
}

function _attachScroll(wheelId, values, onSnap) {
  var wheel = document.getElementById(wheelId);
  if (!wheel) return;
  wheel._snapTimer && clearTimeout(wheel._snapTimer);
  wheel.onscroll = null;
  wheel.addEventListener('scroll', function() {
    clearTimeout(wheel._snapTimer);
    wheel._snapTimer = setTimeout(function() {
      var idx = Math.max(0, Math.min(values.length - 1, Math.round(wheel.scrollTop / 40)));
      wheel.scrollTop = idx * 40;
      onSnap(values[idx]);
    }, 80);
  }, { passive: true });
}

function _applyWheel() {
  var total = _rtMinSel * 60 + _rtSecSel;
  if (total < 5) total = 5;
  setRT(total);
}

function showRT(s) {
  _rtMinSel = Math.floor(s / 60);
  _rtSecSel = Math.round((s % 60) / 5) * 5;
  if (_rtSecSel >= 60) { _rtMinSel++; _rtSecSel = 0; }
  if (_rtMinSel > 5) { _rtMinSel = 5; _rtSecSel = 0; }
  setRT(s);
  _initWheels();
